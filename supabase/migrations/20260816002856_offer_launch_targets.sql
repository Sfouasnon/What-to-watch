-- Platform-specific launch artifacts are intentionally separated from the
-- availability cache. A provider can expose different web, Android TV, and
-- Fire TV targets for the same offer, and none is executable in the product
-- until it has been verified on a real device.

create table public.offer_launch_targets (
  id uuid primary key default gen_random_uuid(),
  availability_offer_id uuid not null references public.availability_offers(id) on delete cascade,
  platform text not null check (platform in ('web', 'android_tv', 'fire_tv')),
  target_kind text not null check (target_kind in ('uri', 'android_intent_uri')),
  target_uri text not null check (
    length(target_uri) between 1 and 4096
    and target_uri ~ '^(https?://|intent:)'
  ),
  package_name text check (package_name is null or package_name ~ '^[A-Za-z0-9_]+(\.[A-Za-z0-9_]+)+$'),
  component_name text check (component_name is null or length(component_name) between 1 and 300),
  action text check (action is null or action ~ '^[A-Za-z0-9_]+(\.[A-Za-z0-9_]+)+$'),
  content_specific boolean not null default true,
  external_source text not null check (external_source ~ '^[a-z0-9][a-z0-9-]{0,120}$'),
  observed_at timestamptz not null,
  expires_at timestamptz not null,
  verification_status text not null default 'unverified'
    check (verification_status in ('unverified', 'verified', 'rejected')),
  verified_at timestamptz,
  verification_notes text check (verification_notes is null or length(verification_notes) <= 1000),
  source_payload jsonb not null default '{}',
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (availability_offer_id, platform, external_source),
  check (expires_at > observed_at),
  check (
    (verification_status = 'verified' and verified_at is not null and content_specific)
    or (verification_status <> 'verified' and verified_at is null)
  )
);

create index offer_launch_targets_active_lookup_idx
  on public.offer_launch_targets(availability_offer_id, verification_status, expires_at desc);

create trigger offer_launch_targets_set_updated_at before update on public.offer_launch_targets
  for each row execute function public.set_updated_at();

alter table public.offer_launch_targets enable row level security;
revoke all on table public.offer_launch_targets from public, anon, authenticated;
grant select, insert, update, delete on table public.offer_launch_targets to service_role;
  ...
  loop
    ...
    loop
      ...
      if uri_value is null or length(uri_value) not between 1 and 4096
        or uri_value !~ '^(https?://|intent:)' then
