...
Fire TV Comrade capabilities are the authority for providers that use a scalar content ID instead of a URI. Record those package/class/action/extra contracts with `npm run catalog:record-fire-tv-extra`; the command is dry-run by default and stores a 30-day unverified target only with `--write`. Promote it separately after a controlled cold-start replay opens the exact movie or episode.
